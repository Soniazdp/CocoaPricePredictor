...
for (i in seq(from = train_window, to = (nrow(full_data) - test_window), by = test_window)) {
  ...
  # Fit the GARCH model
  sarimax_model <- ugarchfit(spec = garch_spec, train_window_data$price_usd_per_tonne_mean,)
  ...
  # Forecast the target variable using GARCH model
  forecast_sarimax <- ugarchforecast(sarimax_model, n.ahead = test_window)
  predicted_values <- as.numeric(fitted(forecast_sarimax))
  
  # Evaluate performance
  rmse_sarimax <- rmse(test_window_data$price_usd_per_tonne_mean, predicted_values)
  mae_sarimax <- mae(test_window_data$price_usd_per_tonne_mean, predicted_values)
  mse_sarimax <- mse(test_window_data$price_usd_per_tonne_mean, predicted_values)
...
plot(time_labels, sarima_results$actual,
     ...
     main = "ARIMA Forecast vs. Actual Prices"
)
